...
void BubbleSort (T *arr, int size) // Функция пузырьковой сортировки. Не возвращает никакого значения так как работает с указателем на массив "arr". Целочисленная переменная "size" принимает размер массива.
{
    F temp; // Временная переменная для хранения переставляемого значения.
    while (size) // Так как смысл пузырьковой сортировки переносить наибольшее число в конец массива после каждой итерации - будет логично
        // предположить что после первой же итерации наибольший элемент будет в конце и более в проверке не нуждается.
        // Сократим процесс сортировки уменьшая переменную размера массива (size) на единицу, тем самым укорачивая оставшийся массив за счёт отделения отсортированной части.
        // (На исходную переменную хранящую размер массива - это не повлияет, так как мы передаём её в функцию по значению - следовательно работаем с её копией).
    {
        for (int i = 0; i < size-1; ++i) { // Цикл прохода по массиву. Так как каждый элемент массива сравнивается со следующим за ним, количество итераций на единицу меньше размера массива (size-1).
            if (arr[i] > arr[i + 1]) { // Проверяем является ли следующий элемент меньше текущего, если нет - игнорируем. Если да - переставляем элементы местами.
                temp = arr[i]; // Сохраняем текущий элемент во временную переменную.
                arr[i] = arr[i+1]; // На место текущего элемента - вставляем следующий за ним.
                arr[i+1] = temp; // Помещаем значение текущего элемента из временной переменной в следующий элемент. Тем самым переставляем местами меньший с большим элементы массива
                // До тех пор, пока наибольший элемент не окажется в конце массива.
            }
        }
        --size; // Сокращаем размер массива на единицу.
    }
}
...
void IntRundomCountOfElementsTest(int size) // Функция создаёт и заполняет массив заданного размера.
{
    for (int i = 0; i < 3 ; ++i) { // Цикл реализует разные варианты заполнения массива в зависимости.
        ...
        ShowArray(arr, size);
        BubbleSort<int, int>(arr, size); // Производим сортировку.
